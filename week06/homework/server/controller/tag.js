...
async function delTags(ctx) {
  try {
    ...
    if (!ids || !Array.isArray(ids) || ids.length === 0) {
      throw new Error('ids参数必须是非空数组');
    }

    // 验证所有ID是否都被使用
    const { data } = await dataService.getData();
    const usedIds = data.flatMap(item => item.tags).filter(id => ids.includes(id));
    
    if (usedIds.length > 0) {
      throw new Error(`以下ID已被使用，不能删除: ${usedIds.join(', ')}`);
    }

    // 执行批量删除操作
    const result = await tagsService.delTags(ids);
