...
async function addData(ctx) {
  try {
    const { name = '', description = '', tags = [], url = 'https://s21.ax1x.com/2024/07/31/pkOgGH1.png' } = ctx.request.body;
    if (!name || !description) {
...
async function editData(ctx) {
  try {
    const { id, name , description, tags, url } = ctx.request.body;
    ...
    // 调用服务模块的新增数据函数
    await dataService.editData(id, name , description, tags, url);
